...
We implement FastICA according to the formalism described in the previous section and apply it to the observed data $X$ in order to find the source matrix $S$ given by
\begin{align}
S &= \begin{bmatrix}
...
\end{align}
The mixture signals $X$ (Fig.~\ref{fig:ex1_sources}(b)) that we want to apply FastICA on is constructed by the equation
\begin{align}
X &= AS,
\end{align}
where $A$ is the mixing matrix, which in this example, is set to be
\begin{align}
A &= \begin{bmatrix}
1 & 1 & 1 \\
\frac{1}{2} & 2 & 1 \\
...
\end{align}
Newton's method converages rather quickly within only four cycles for each row of $W$ as shown in Fig.~\ref{fig:ex1_dist}.
\begin{figure}
\includegraphics{scripts/ex1_dist.pdf}
\caption{\label{fig:ex1_dist} The convergence criterion $||w_{n}^T w_{n-1}| - 1|$ as a function of cycle for each row of $W$.}
\end{figure}
...
From the predicted sources $S_{\mathrm{predicted}}$, we can verify the accuracy of FastICA results by inverting the mixing and whitening transformations and adding back the mean of each mixture to retrieve the input signals.
\begin{align}
(X_{\mathrm{retrived}})_{i:} &= [(W K)^{-1} S_{\mathrm{predicted}}]_{i:} + E[X_{i:}], \label{eq:ex1_retrieved}
\end{align}
where $K$ is the whitening transformation given by...
The retrieved signals $X_{\mathrm{retrieved}}$ is shown to be equal to the original input signal $X$ (Fig.~\ref{fig:ex1_sources}(c)), which confirms that we can exactly reproduce the observed data from the output of ICA.
\begin{figure*}
...
\end{figure*}
In some applications, such as separation of independent sound sources, we might not need to retrieve the original signal. However, this procedure becomes crucial in other applications, such as blink removals of the electroencephalogram data. The discussion of both mentioned applications follows.
